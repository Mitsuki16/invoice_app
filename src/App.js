...
import InvoiceDetails from "./InvoiceList/InvoiceDetails";
import { Route, Routes, useNavigate } from "react-router-dom";
import InvoiceHomePage from "./InvoiceList/InvoiceHomePage";
...
const INVOICE_DATA = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Brand Guidelines", quantity: 1, price: 2500, total: 2500 },
    ],
    total: 4032.33,
  },
  {
...
function App() {
  ...
  const onStatusUpdate = (id) => {
    ...
    console.log(item);
    console.log({ ...item[0], status: "paid" });
    let changedItem = { ...item[0], status: "paid" };
    let fulldata = data.filter((item) => {
      return item.id != id;
    });
    setData([...fulldata, changedItem].sort((a, b) => (a.id > b.id ? 1 : -1)));
  };

  const onInvoiceDelete = (id) => {
    let fulldata = data.filter((item) => {
      return item.id != id;
    });
    setData([...fulldata]);
  ...
  return (
    <div className="  bg-violet-200  dark:bg-[#141625] h-[100vh]">
      <Routes>
        <Route
          path="/invoice/:id"
          element={
        ...
        <Route
          path="/invoices"
          element={<InvoiceHomePage data={data} />}
        ></Route>
        <Route
          path="/"
          element={<InvoiceHomePage data={data} />}
